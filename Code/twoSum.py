# -*- coding: utf-8 -*-
#两数之和：
# 要解决这个问题，我们需要在一个整数数组中找到两个数，使得它们的和等于给定的目标值，并返回它们的数组下标。我们可以通过使用哈希表来优化查找过程，从而实现高效的时间复杂度。

# 方法思路
# 问题分析：我们需要找到两个不同的元素，它们的和等于目标值。直接暴力解法的时间复杂度为O(n²)，不够高效。使用哈希表可以将时间复杂度优化到O(n)。

# 哈希表使用：遍历数组，对于每个元素，计算其补数（即目标值减去当前元素值）。检查补数是否存在于哈希表中。如果存在，则返回补数的下标和当前元素的下标；如果不存在，则将当前元素的值及其下标存入哈希表。
