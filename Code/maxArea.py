...
# 代码解释
# 初始化指针：left 初始化为 0，right 初始化为数组的最后一个索引。
# 循环计算面积：在每次循环中，计算当前两个指针之间的宽度和高度，进而计算当前面积，并更新最大面积。
# 移动指针：根据两个指针所指的高度，移动高度较小的指针，以寻找可能的更大面积。这一步骤确保了每次移动都有可能在后续步骤中找到更高的高度，从而优化结果。
# 这种方法通过双指针的移动策略，高效地遍历可能的垂线组合，确保在 O(n) 时间复杂度内找到最优解。